...
// Read the YAML files
function readYamlFile(filePath: string): Extensions {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    return yaml.load(content) as Extensions;
  } catch (error) {
...
function getExistingVersion(extensionDir: string, extension: Extension): string {
  ...
  try {
    if (fs.existsSync(packagePath)) {
      ...
      if (packageJson.version) {
        ...
        const existingExtensions = (packageJson.extensionPack || []) as string[];
        ...
        if (totalExtensions === 0) return packageJson.version; // No extensions, no changes
...
function generateExtensionFiles(
  extensionKey: string,
  extension: Extension,
  outputDir: string,
  packageTemplate: Handlebars.TemplateDelegate,
  readmeTemplate: Handlebars.TemplateDelegate,
  licenseContent: string
) {
  const extensionDir = path.join(outputDir, `itmcdev-${extensionKey}-extension-pack`);
  
  // Create directory if it doesn't exist
  mkdirp.sync(extensionDir);
  ...
  // Copy LICENSE file
  fs.writeFileSync(
    path.join(extensionDir, 'LICENSE'),
    licenseContent,
    'utf8'
  );
